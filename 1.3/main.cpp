...
int main(int argc, char const *argv[])
{
	...
	for(double i = 0; i < 10/F;i+=0.0001)
	{
		res_t.push_back(i);
	}
...
/*	std::vector<std::pair<double, double>> recovery1 = signalRecovery(1/(1.5*F), 10/F, sample1);
	...
	std::ofstream  fs("Signaly.txt");
	std::ofstream  fs1("Signalx.txt");
	for (std::vector<double>::iterator it = res_t.begin(); it != res_t.end(); ++it){
		fs<< ", ";
		fs<<sin(3*F * (*it) + M_PI/3);
		fs1<< ", ";
		fs1<< (*it);
	}
	fs.close();

	std::ofstream frs("RecoverySignaly.txt");
	std::ofstream frs1("RecoverySignalx.txt");
	...
	for (int i = 0; i < recovery4.size(); i++)
	{
		...
		frs<< ", ";
		frs<<recovery4[i].second;
		frs1<<", ";
		frs1<<recovery4[i].first;
	}
		frs.close();
	std::cout << std::endl << std::endl << "Press Enter to resume..." << std::endl << std::endl;
	getch();
